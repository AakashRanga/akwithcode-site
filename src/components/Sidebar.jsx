    ...
    <aside className="lg:col-span-4 space-y-12">
      ...
      <section>
        ...
        <div className="flex flex-wrap gap-2">
          {["#Kubernetes", "#Web3.0", "#DevOps", "#AIGovernance", "#ZeroTrust", "#Quantum"].map(topic => (
            <a key={topic} className="bg-surface-dark border border-primary/20 px-3 py-2 text-[10px] font-bold uppercase tracking-widest hover:bg-primary hover:text-background-dark transition-all cursor-pointer" href="#">{topic}</a>
          ))}
        </div>
      </section>
      ...
      <section className="bg-accent p-6 text-white relative overflow-hidden">
        <div className="absolute -right-4 -bottom-4 opacity-10 pointer-events-none">
          <span className="material-symbols-outlined text-[120px] rotate-12 select-none">sensors</span>
        </div>
        ...
        <div className="space-y-3">
          ...
          <button className="w-full bg-white text-accent font-black py-3 text-xs uppercase tracking-[0.2em] hover:bg-background-dark hover:text-white transition-colors cursor-pointer">Subscribe</button>
        </div>
      </section>
    </aside>
  );
