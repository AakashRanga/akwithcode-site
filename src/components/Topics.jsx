...
const Topics = () => {
  ...
  return (
    <div className="flex-1 px-4 sm:px-6 lg:px-20 py-12 max-w-7xl mx-auto w-full relative">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 mb-12 scrollbar-hide border-b border-primary/10">
        {categories.map((cat, i) => (
          <button key={i} className={`flex-none flex items-center gap-2 px-5 py-2 ${i === 0 ? 'bg-primary/20 border-primary/40 text-primary' : 'hover:bg-primary/10 border-transparent hover:border-primary/20 text-slate-500'} rounded-full border transition-all font-bold text-sm text-nowrap cursor-pointer`}>
            {i === 0 && <span className="material-symbols-outlined text-sm select-none">category</span>}
            {cat}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {topicsList.map((topic, i) => (
          <div key={i} className="group relative aspect-square bg-slate-900 overflow-hidden rounded-xl border-2 border-slate-800 transition-all duration-300 hover:-translate-y-2 glow-border flex flex-col justify-end p-4 sm:p-8">
            <div className="absolute inset-0 z-0 transition-transform duration-500 group-hover:scale-110">
              <img className="w-full h-full object-cover opacity-50 grayscale group-hover:grayscale-0 select-none" src={topic.img} alt={topic.title} />
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-xl sm:text-2xl md:text-3xl font-black text-white uppercase tracking-tighter leading-tight mb-2 whitespace-pre-line">
                {topic.title}
              </h3>
              ...
              <div className="flex items-center gap-2 text-primary text-sm font-bold uppercase tracking-widest cursor-pointer">
